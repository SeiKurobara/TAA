...
public class stg22EnemyDamage : MonoBehaviour
{
    ...
    public void DamageEnemy()
    {
        ...
        if (attackCooldown.Equals(0))
        {
            if (Input.GetKeyDown(KeyCode.Mouse0))
            {
                ...
                {
                    if (enemyObj.GetComponent<stg22EnemyHealth>().enemyDied == false)
                    {
                        StartCoroutine(TimeDelay());
                        enemyObj.GetComponent<stg22EnemyHealth>().AddDamage(enemyDamageAmount);
                        attackCooldown = 1.4f;
                        Debug.Log("Enemy Hit");
