...
public class stg12EnemyHealth : MonoBehaviour
{
    ...
    public void AddDamage(float damage)
    {
        ...
        if (currentHealth <= 0)
        {
            enemyDied = true;
            MakeDied();
            FindObjectOfType<stg12Score>().Stg12ScorePointIncrease(enemyPlusScoreWhenDead);
            Instantiate(enemyDeathEffect, starSpawn.transform.position, starSpawn.transform.rotation);
