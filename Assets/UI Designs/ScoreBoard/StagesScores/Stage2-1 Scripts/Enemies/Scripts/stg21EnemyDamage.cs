...
public class stg21EnemyDamage : MonoBehaviour
{
    ...
    public void DamageEnemy()
    {
        ...
        if (attackCooldown.Equals(0))
        {
            if (Input.GetKeyDown(KeyCode.Mouse0))
            {
                ...
                {
                    if (enemyObj.GetComponent<stg21EnemyHealth>().enemyDied == false)
                    {
                        StartCoroutine(TimeDelay());
                        enemyObj.GetComponent<stg21EnemyHealth>().AddDamage(enemyDamageAmount);
                        attackCooldown = 1.4f;
                        Debug.Log("Enemy Hit");
